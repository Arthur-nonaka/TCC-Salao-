...
function ComboBox({ data, columnToTake, setDataSelected, dataSelected }) {
  ...
  const handleValueClick = (value) => {
    if (typeof dataSelected !== "string") {
      ...
      if (x) setDataSelected([...dataSelected]);
      else setDataSelected([...dataSelected, value]);
    } else setDataSelected(value);
    setInput("");
  };
  ...
  if (isFocused === true) {
    const updatedData = data.filter((value) =>
      value[columnToTake].toLowerCase().includes(input.toLowerCase())
    );
    ...
    const values = sortedData.map((value, index) => {
      return (
        <div
          onClick={() => handleValueClick(value[columnToTake])}
          key={index}
          className="value"
        >
          {value[columnToTake]}
    ...
    content = <div className="options-comboBox">{values}</div>;
  }
  return (
