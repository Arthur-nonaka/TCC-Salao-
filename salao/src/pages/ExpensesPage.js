...
function ExpensesPage() {
    ...
    useEffect(() => {
        axios.post('/pull/Despesas', { email })
            .then(res => {
    ...
    const registerPage = <div>
        <div className="form-group w-auto me-1" >
            ...
            <input type="text" className="form-control p-2 input" placeholder="Nome" onChange={(event) => setDesc(event.target.value)} value={desc} />
        </div>
        <div className='me-1'>
        ...
        <div className="form-group w-auto me-1" >
            ...
            <input type="date" className="form-control p-2 input" onInput={(event) => setDate(event.target.value)} />
        </div>
    </div>;
    ...
    return (
        <div className='main'>
            ...
            <SearchTerm data={expenses} config={config} size={"10000px"} type={type} handleReset={handleReset} />
